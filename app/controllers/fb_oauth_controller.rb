...
# 
   ...
   def set_session
     puts "i am in setsession with#{params}"
     session[:token]= params[:user_access_token]
      session[:user_id]=""
     # session[:id]= params[:id]
     client = FBGraph::Client.new(:client_id => '641206265895621',:secret_id =>'e81cd8aa1272a4b3a17e3ac0824b07dc',:token => params[:user_access_token])
     user_info = client.selection.me.info!
     @user = User.where(:fb_id => user_info.id).first()
     if not @user
      @user          = self.create(params[:user_access_token],user_info.first_name, user_info.id,user_info.email)
      ...
      session[:user_id]=@user.id
      @user.update_attributes({:facebook_token => params[:user_access_token], :date_token_updated => Time.now})
    end
     self.session_counter(@user.id)
# 
    if $admins.include? @user.fb_id.to_i
     ...
     puts "session after setting in set session#{user_info.id}"
     respond_to do |format|
